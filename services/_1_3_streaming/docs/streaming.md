...
|---|---|---|
| Chat UI where user is watching | ✅ Yes | ❌ No |
...
*Structured JSON is tricky to stream because partial JSON cannot be parsed. The full streamed response is typically buffered, then parsed. (This is exactly what Phase 1.4 covers.)
...
**Important:** Only append the *completed* assistant message to the conversation history after the stream finishes — not partial chunks. The history array should always contain complete messages.

---

### Time to First Token (TTFT)

One of the key metrics in streaming systems is **Time to First Token (TTFT)**, the time it takes from sending the request until the first chunk arrives.
...
- [x] Understand what SSE is and how it works at the HTTP level
- [x] Know the structure of a streaming chunk (`delta`, `finish_reason`)
- [x] Correctly accumulate chunk deltas into a full response string
- [x] Understand what backpressure is and why it matters
- [x] Measure TTFT and understand what affects it
- [x] Handle mid-stream errors and partial responses gracefully
- [x] Integrate streaming cleanly with conversation history
- [x] Know when streaming is the right choice vs buffered responses
