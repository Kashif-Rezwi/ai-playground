...
// Prevents infinite agentic loops if the model gets stuck calling tools forever.
...
2. If multiple pieces of information are INDEPENDENT of each other, call all necessary tools in parallel.
3. If a tool call depends on the RESULT of a previous tool call, call them sequentially — one at a time.
4. If a tool returns an error, gracefully explain the issue to the user using the available information.
5. Never guess or make up data for weather, time, or exchange rates.`;
